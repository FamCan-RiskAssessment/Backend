...
func SetupAdminRoutes(routerGroup *gin.RouterGroup, app *wire.Application) {
	...
	operators.Use(app.Middlewares.Auth.RequiredWithPermission([]enum.PermissionType{enum.PermissionCreateFormForUser}))
	{
		...
		operatorFormGroup := operators.Group("/form/:formID")
		{
			...
			operatorFormGroup.PUT("/request-response", app.Controllers.Admin.FormController.RequestPatientResponse)
			operatorFormGroup.PUT("/request-documents", app.Controllers.Admin.FormController.RequestDocuments)
	...
	operatorData.Use(app.Middlewares.Auth.RequiredWithPermission([]enum.PermissionType{enum.PermissionFillProfile}))
	{
	...
	forms.Use(app.Middlewares.Auth.RequiredWithPermission([]enum.PermissionType{enum.PermissionType(enum.CategoryFormManagement)}))
	{
		...
		supervisorFormManagement.Use(app.Middlewares.Auth.RequiredWithPermission([]enum.PermissionType{enum.PermissionType(enum.PermissionHandleOperators)}))
		{
			...
			supervisorFormManagement.PUT("/request-response", app.Controllers.Admin.FormController.RequestPatientResponse)
			supervisorFormManagement.PUT("/request-documents", app.Controllers.Admin.FormController.RequestDocuments)
		}
